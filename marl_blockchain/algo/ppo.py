...
@jax.jit
def rollout(rng_key, env, policy_fn, state, num_steps):
...
def train_ppo(env, num_steps, num_envs, num_epochs, batch_size, lr, gamma, lambda_, clip_ratio):
    ...
    ppo_state = PPOState(policy_params=policy_params, value_params=value_params, opt_state=opt_state, rng_key=rng_key)
    ...
    for epoch in range(num_epochs):
        ...
        rng_key, subkey = jax.random.split(rng_key)
        states, actions, rewards, dones = rollout(subkey, env, policy_net.apply, dummy_state, num_steps)

        # Compute the returns and advantages
        last_value = value_net.apply(value_params, states[-1])
        returns = jax.vmap(lambda r, v, d: r + gamma * v * (1.0 - d))(rewards, last_value, dones)
        advantages = compute_gae(rewards, jax.vmap(value_net.apply, states), dones, last_value, gamma, lambda_)

        # Add the collected experiences to the buffer
        buffer.extend(zip(states, actions, returns, advantages))
